function data_out = SaveImg2Amap(fname_in,fname_amap,fname_phg_act_table,n_bins,img_size,vox_size,fname_log)
%SAVEIMG2AMAP Summary of this function goes here
%   Detailed explanation goes here

if nargin ~= 7
    error('Usage: SaveImg2Amap(fname_in,fname_amap,fname_phg_act_table,n_bins,img_size,vox_size,fname_log)');
end
pause(0.1);

fid_log = fopen(fname_log, 'a+');

fprintf('Processing %s...\n',fname_in);
fprintf(fid_log, 'Processing %s...\n',fname_in);
fprintf('Image size: %d, %d, %d', img_size(1), img_size(2), img_size(3));
fprintf(fid_log, 'Image size: %d, %d, %d\n', img_size(1), img_size(2), img_size(3));
...
s=size(data_in);
fprintf(fid_log, 'data size (float): %d\n', s(1));
