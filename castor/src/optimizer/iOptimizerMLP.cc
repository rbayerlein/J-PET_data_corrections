...
/*!
  \file
  \ingroup  optimizer
  \brief    Implementation of class iOptimizerMLP
*/

#include "iOptimizerMLP.hh"
...
iOptimizerMLP::iOptimizerMLP() : vOptimizer()
{
  // ---------------------------
  ...
  m_initialValue = 1.;
  // We don't want to backward image
  ...
  //probably should be possible, but for now, lets disable histogram data
  ...
  // not sure, but I think, only emmission
...
void iOptimizerMLP::ShowHelpSpecific()
{
  cout << "This optimizer is MLP. It ignores sensitivity map and doesn't change with iteration number. It should be used with Projector MLP, but due to CASToR limitation it still applies constant backprojection correction." << endl;
}
...
int iOptimizerMLP::ReadConfigurationFile(const string& a_configurationFile)
{
...
int iOptimizerMLP::ReadOptionsList(const string& a_optionsList)
{
...
int iOptimizerMLP::CheckSpecificParameters()
{
...
int iOptimizerMLP::InitializeSpecific()
{
  // Verbose
  if (m_verbose>=2) Cout("iOptimizerMLP::InitializeSpecific() -> Use the MLP optimizer" << endl);
...
int iOptimizerMLP::PreImageUpdateSpecificStep()
{
...
int iOptimizerMLP::SensitivitySpecificOperations( FLTNB a_data, FLTNB a_forwardModel, FLTNB* ap_weight,
...
int iOptimizerMLP::DataSpaceSpecificOperations( FLTNB a_data, FLTNB a_forwardModel, FLTNB* ap_backwardValues,
                                                     FLTNB a_multiplicativeCorrections, FLTNB a_additiveCorrections, FLTNB a_blankValue,
                                                     FLTNB a_quantificationFactor, oProjectionLine* ap_Line )
{
...
int iOptimizerMLP::ImageSpaceSpecificOperations( FLTNB a_currentImageValue, FLTNB* ap_newImageValue,
                                                      ...
                                                      INTNB a_voxel, int a_tbf, int a_rbf, int a_cbf )
{
  ...
  // Skip sensitivity map
  *ap_newImageValue = *ap_correctionValues;
