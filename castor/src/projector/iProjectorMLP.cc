...
/*!
  \file
  \ingroup  projector
  \brief    Implementation of class iProjectorMLP
*/
...
iProjectorMLP::iProjectorMLP() : vProjector()
{
...
int iProjectorMLP::ReadConfigurationFile(const string& a_configurationFile)
{
  // No options for MLP
  ;
  // Normal end
...
int iProjectorMLP::ReadOptionsList(const string& a_optionsList)
{
  // No options for MLP
  ;
  // Normal end
...
void iProjectorMLP::ShowHelpSpecific()
{
  cout << "This projector is a MLP projector." << endl;
}
...
int iProjectorMLP::CheckSpecificParameters()
{
...
int iProjectorMLP::InitializeSpecific()
{
  // Verbose
  if (m_verbose>=2) Cout("iProjectorMLP::InitializeSpecific() -> Use MLP projector" << endl);
  // Normal end
  return 0;
}
...
INTNB iProjectorMLP::EstimateMaxNumberOfVoxelsPerLine()
{
...
int iProjectorMLP::ProjectWithoutTOF(int a_direction, oProjectionLine* ap_ProjectionLine )
{
  #ifdef CASTOR_DEBUG
  if (!m_initialized)
  {
    Cerr("***** iProjectorMLP::ProjectWithoutTOF() -> Called while not initialized !" << endl);
    Exit(EXIT_DEBUG);
  }
  #endif
  ...
  if (m_verbose>=10)
  {
    ...
    Cout("iProjectorMLP::Project without TOF -> Project line '" << ap_ProjectionLine << "' in " << direction << " direction" << endl);
  }
  #endif
  ...
  for( int nP = 0; nP < n - 1; ++nP, ++pAlpha, ++pAlphaPrevious )
  {
    ...
    ap_ProjectionLine->AddVoxel(a_direction, numVox, 1);
  }
  ...
  if (m_verbose>=10)
  {
    Cout("iProjectorMLP::Project without TOF -> Exit function" << endl);
  }
  #endif
...
int iProjectorMLP::ProjectTOFListmode(int a_direction, oProjectionLine* ap_ProjectionLine)
{
  #ifdef CASTOR_DEBUG
  if (!m_initialized)
  {
    Cerr("***** iProjectorMLP::ProjectTOFListmode() -> Called while not initialized !" << endl);
    Exit(EXIT_DEBUG);
  }
  #endif
  ...
  if (m_verbose>=10)
  {
    ...
    Cout("iProjectorMLP::Project with TOF measurement -> Project line '" << ap_ProjectionLine << "' in " << direction << " direction" << endl);
  }
  #endif
  ...
  for( int nP = 0; nP < n - 1; ++nP, ++pAlpha, ++pAlphaPrevious )
  {
    ...
    // if this voxel is masked, skip
    if (!m_hasMask || mp_mask[numVox]) ap_ProjectionLine->AddVoxel(a_direction, numVox, 1);
  ...
  if (m_verbose>=10)
  {
    Cout("iProjectorMLP::Project with TOF measurement-> Exit function" << endl);
  }
  #endif
...
int iProjectorMLP::ProjectTOFHistogram(int a_direction, oProjectionLine* ap_ProjectionLine)
{
  #ifdef CASTOR_DEBUG
  if (!m_initialized)
  {
    Cerr("***** iProjectorMLP::ProjectTOFHistogram() -> Called while not initialized !" << endl);
    Exit(EXIT_DEBUG);
  }
  #endif
  ...
  if (m_verbose>=10)
  {
    ...
    Cout("iProjectorMLP::Project with TOF bins -> Project line '" << ap_ProjectionLine << "' in " << direction << " direction" << endl);
  }
  #endif
  ...
  if (m_verbose>=10)
  {
    Cout("iProjectorMLP::Project with TOF bins -> Exit function" << endl);
  }
  #endif
