...
bool ConvertEvents::exec()
{
  if (const auto& timeWindow = dynamic_cast<const JPetTimeWindow* const>(fEvent))
  {
    ...
    for (unsigned int i = 0; i < numberOfEventsInTimeWindow; i++)
    {
      ...
      uint32_t castor1 = ConvertEventsTools::getCastorID(firstHit.getPosX()  * 10, firstHit.getPosY()  * 10, firstHit.getPosZ()  * 10, fCastorIDs, fScintillatorLength, fCrystalSizeZ);
      uint32_t castor2 = ConvertEventsTools::getCastorID(secondHit.getPosX() * 10, secondHit.getPosY() * 10, secondHit.getPosZ() * 10, fCastorIDs, fScintillatorLength, fCrystalSizeZ);
